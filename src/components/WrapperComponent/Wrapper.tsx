...
class Wapper extends Component<{},WrapperState> {
    ...
    calculateResult = () => {
        const mexp = new Mexp();
        const evaluatedResult = mexp.eval(this.state.display);
    ...
    handleClick = (e:any)=>{
        ...
        if(char =='AC'){
            ...
            if(this.state.result != 0) {
                flushSync(()=>{this.setState({display: String(this.state.result)+ char,result:0})});
            }
            else {
                if(operations.has(char)){
                    ...
                    else if(operations.has(this.state.display[this.state.display.length-1]) && char !== '-')
                        flushSync(()=>{this.setState({display: this.state.display.slice(0,-1) + char,decimalApplied:false, leftZero:true})}) 
                    else
                        flushSync(()=>{this.setState({display: this.state.display + char,decimalApplied:false, leftZero:true})}) 
                }
                else {
